...
在 MVP 阅读理解闭环中，回答方式从「点击选项」扩展为「语音说出答案」。核心思路：

- 孩子用自己的话说出答案，而不是在选项里选。
- 后端把**原文 / 对应段落、题目、参考答案、语音转写文本**一起交给 LLM，由 LLM 判断答案是否正确，并给出孩子能看懂的简短反馈。
- 阅读器新增**跟读**：AI 读一句，孩子跟读，后端用 ASR 转写并与目标句比对，反馈「读对了 / 还差一点」。
- 本阶段**不做发音评测**（音素级打分），后续需要时再引入 Azure 发音评估或讯飞等发音评测服务。
...
读完故事后进入答题页。题目直接展示，孩子**按住麦克风用自己的话说出答案**，不用点选项。
...
1. 前端录音，把音频发给后端 ASR 转成文字。
2. 后端把「原文 / 对应段落 + 题目 + 参考答案 + 转写文本」一起交给 LLM。
3. LLM 返回结构化结果：`correct`、`feedback`、`model_answer`。
4. 前端展示对错 + 简短鼓励/纠正，再进入下一题。

MVP 规则：

- 3～5 道题
- 孩子用自己的话回答，允许语法小错，关键意思答对即算对
- 麦克风不可用或识别失败时，降级为键盘输入答案，不强制语音
...
阅读器页面新增「跟读」按钮，点开后进入逐句跟读模式：

```text
← The Little Dog          跟读模式

AI 读一句：
The dog is very cute.     [🔊 再听一遍]

孩子跟读：
        [ 🎙 按住说话 ]

转写结果：The dog is very cute.
...
- 一次只跟读一句，句子短、反馈快。
- 后端用 ASR 转写孩子语音，与目标句做文本比对。
- 本阶段只反馈「读对了 / 还差一点」，不做音素级发音评分。
- 发音评测（Azure 发音评估 / 讯飞）作为后续增强，不在本阶段实现。
...
`judge_answer` 是语音答题的核心：输入原文/段落、题目、参考答案、用户语音转写文本，输出对错判断和儿童友好的简短反馈。

语音链路依赖 **ASR**（语音转文字）。本阶段先用通用 ASR（如 Whisper 或云 ASR）做转写；发音评测（音素级打分）不作为本阶段目标。
...
- `correct`：是否答对（关键意思到位即可，语法小错不算错）
- `feedback`：给孩子看的简短反馈，中文为主，只纠正理解错误
- `model_answer`：参考答案/示范表达
...
Phase 7 聚焦「让孩子开口说」，分两步落地，发音评测暂不做：
...
- 新增 `judge_answer` AI 服务（见 11.3 / 11.5）。
- 新增 ASR 服务抽象：先接通用 ASR（Whisper / 云 ASR），音频上传后端处理。
- 题目沿用现有 Quiz 表；答题方式从选项改为语音 + 开放回答。
- 麦克风不可用时降级为键盘输入。
...
- 复用 Phase 7A 的 ASR 链路。
- 先做文本级比对，不做音素级发音评分。
- 云 TTS（Azure / Google）用于提升朗读音色，按需接入。
...
动画、游戏化、个性化故事等继续保留为后续候选，不在本阶段实现。
